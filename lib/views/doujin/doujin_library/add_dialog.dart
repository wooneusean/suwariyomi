...
class _AddDialogState extends State<AddDialog> {
  ...
  SnackBar successSnackBar(doujin) => SnackBar(
        content: Text(
          (doujin?.id!.toString())! + " - " + (doujin?.title!.pretty) + " added successfully",
  ...
  Widget build(BuildContext dialogContext) {
    AlertDialog dialog = AlertDialog(
      ...
      actions: [
        TextButton(
          child: Text('SUBMIT'),
          onPressed: _nukeCode.text.isNotEmpty && isButtonActive
              ? () async {
                  try {
                    setState(() {
                    ...
                    ScaffoldMessenger.of(dialogContext).showSnackBar(successSnackBar(doujin));
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(failedSnackBar(_nukeCode.text.trim(), e.toString()));
